...
Install the CLI with `python3 -m pip install git+https://github.com/replitprivet-dotcom/co08-ssh-hosts.git#subdirectory=cli`. Create an API key from the dashboard’s **API access** page, then run `co08 login`; the token is entered through a hidden prompt, validated against the service, and stored at `~/.config/co08/config.json` with mode `600`. Run `co08 ssh --user root --port 22` to list the logged-in user’s existing SSH commands. To create a new host, run `co08 ssh --ip PUBLIC_VPS_IP --user root --port 22`; when hosts already exist, adding `--ip` creates another one and omitting it lists the existing hosts. The CLI asks for the password twice only as a local confirmation step; it never sends or stores that password. Use `--user ubuntu` or another account when that is the account configured on the VPS. Use `co08 logout` to remove the local token.
...
The application never opens an SSH session, executes a shell command, installs packages, or runs code on the VPS. It only returns DNS and connection metadata. A private RFC1918 address cannot be used as a public DNS target; if phone and VPS traffic uses a VPN or private network, use the address reachable from that network and configure firewall/SSH access on the VPS owner’s side.
