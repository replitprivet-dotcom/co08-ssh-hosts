...
import { authenticateApiKey, createManagedHost, expirationFromSeconds } from "../hostService";
...
async function startServer() {
  ...
  app.post("/api/scheduled/cleanup-expired", async (req, res) => { try { const user = await sdk.authenticateRequest(req); if (!user.isCron) return res.status(403).json({ error: "cron-only" }); const rows = await expireHosts(); for (const host of rows) { const dns = await getDnsRecord(host.id); if (dns && !dns.deletedAt) await deleteDnsRecord(dns.cloudflareRecordId).catch(() => undefined); await markDnsDeleted(host.id); await markHost(host.id, "expired"); await addAuditLog({ action: "dns.expire", resource: host.hostname, details: JSON.stringify({ taskUid: user.taskUid }) }); } res.json({ ok: true, expired: rows.length }); } catch (error) { res.status(500).json({ error: error instanceof Error ? error.message : "Cleanup failed", timestamp: new Date().toISOString() }); } });
